...
// LongList is for outputting JSON
...
func printLongDirEntries(de []*upspin.DirEntry) {
	...
	for _, e := range de {
		...
		longList = append(longList, LongList{
			AttrChar:   attrChar,
			PackStr:    packStr,
			SeqWidth:   seqWidth,
			SeqVersion: upspin.SeqVersion(e.Sequence),
			SizeWidth:  sizeWidth,
			SizeOf:     sizeOf(e),
			Time:       e.Time.Go().Local().Format("Mon Jan _2 15:04:05"),
			EndPoint:   endpt,
			Name:       e.Name,
			Redirect:   redirect,
		})
	}
	out, _ := json.Marshal(longList)
